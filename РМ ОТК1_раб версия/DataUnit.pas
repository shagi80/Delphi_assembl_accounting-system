...
const
  //скорость производства в процентах от планового количества
  SpeedBad     = 0.6;   //меньше этого значения - двойка - красное
  SpeedNormal  = 0.75;  //между этим и предыдущим - тройка - желтое
  SpeedGood    = 0.9;   //между этим и предыдущим - хорошо - голубое
                        //выше этого - отлично - зеленое
  SpeedInterval=10;     //интервал замера скорости, мин
  DefEmployCount=23;    //количество персонала в смене по умолчанию
  DefTempMax = 90;      //время на одну стиралку по умолчанию
...
var
  ...
  FaultProc   : real = 0;            //текущий процент брака
  ...
  //задается в файле инициализации
  ...
  HourBonus   : array of integer;    //размер премии за выполнение плана за час
  DayBonus    : integer=0;           //дневной бонус
  FaultBonus  : integer = 0;         //базовая премия за брак
  HourBonusSum: real=0;              //текущий размер премии за выполнение плана за час
  DayBonusSum : real=0;              //текущий дневной бонус
  FaultBonusSum: real=0;             //текуий  премия за брак
  RedgProcBad  : real = 5;           //выше этого значения - очень плохой
  RedgProcNorm : real = 3;           //до этого значения - нормальный

  StopTimeMax : byte=10;             //макс время простоя, мин
  FaultToFifteenMin : real = 3;      //макс количество брака за пятнадцать минут
  FaultToHour       : real = 10;     //макс колиечетво брака за час
  OTK1cntPay,OTK1faultPay : real;    //оплата контролера №1 (за единицу, за брак)
  OTK2cntPay,OTK2faultPay : real;    //оплата контролера №2 (за единицу, за брак)
...
begin
  ...
  Models.Free;
  sum:=sum+(HourBonusSum+DayBonusSum+FaultBonusSum)*EmployCount;
  result:=sum;
end;
